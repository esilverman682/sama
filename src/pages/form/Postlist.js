...
export default function Postlist() {
  ...
  const fetchPosts = async (cursor = null) => {
    ...
    try {
      ...
      const pageInfo = response.data.data.posts.pageInfo;
  ...
  const handlePostClick = (slug) => {
    router.push(`/form/post?slug=${slug}`); // Navigate to dynamic post page
  };

  return (
    <div className="p-4 m-2">
      <h1 className="text-4xl mb-4">Posts Review Site</h1>
